...
public interface CommentRepository extends JpaRepository<Comment, Long> {
  ...
  @Query(//
      value = "select c from Comment c join fetch c.member m where c.board = :board order by c.createdDate asc", //
      countQuery = "select count(c.id) from Comment c where c.board = :board")
  Page<Comment> findWithMemberByBoard(@Param("board") Board board, Pageable pageable);
